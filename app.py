...
# @app.post("/run_tests")
# async def run_tests_from_yaml(file: UploadFile = File(...)):
#     """
#     Upload a YAML test-definition (with top-level 'tests' list),
#     run each vars.question/image through query_knowledge_base, and
#     return a list of {answer, links} results.
#     """
...
#         image    = vars.get("image")

#         if not question:
#             raise HTTPException(status_code=400,
#                 detail=f"Test #{idx+1} missing 'vars.question'")

#         # 3. Build a QueryRequest and re‑use your existing handler
#         req = QueryRequest(question=question, image=image)
#         resp = await query_knowledge_base(req)

#         results.append(resp)

#     # 4. Return the array of responses
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, json=payload) as response:
                    if response.status == 200:
                        ...
                        logger.warning(f"Proxy failed with status {response.status}. Retrying with OpenAI key...")
                        openai_headers = {
                            "Content-Type": "application/json",
                            "Authorization": f"Bearer {os.getenv('API_KEY')}"
                        }
                        # openai_url = "https://api.openai.com/v1/chat/completions"
                        response = await session.post(url, json=payload, headers=openai_headers)
                        if response.status == 200:
                            result = await response.json()
                            return result["choices"][0]["message"]["content"]
                        else:
                            err = await response.text()
                            raise RuntimeError(f"OpenAI fallback failed: {err}")
        ...
        if len(parts) > 1:
            ...
            for line in source_lines:
                ...
                if url_match:
                    ...
                    # url = next((g for g in url_match.groups() if g), "")
                    # url = url.strip()
                    url = next((g for g in url_match.groups() if g), "").strip().rstrip('.,')
